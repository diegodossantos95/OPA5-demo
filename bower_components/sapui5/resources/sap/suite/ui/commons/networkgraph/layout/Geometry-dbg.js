sap.ui.define(
	...
	function () {
		...
		Geometry.getBezierPathCorners = function (sPath, fRadius, fPerpendicularShift) {
			...
			function fnGetNormalizedVector(oVector, fNewLength) {
			...
			function fnShiftStepsAside(fShift) {
				...
				oShift = fnGetNormalizedVector({center: oCurr, apex: oNext}, fShift);
				...
				oShift = fnGetNormalizedVector({center: oCurr, apex: oPrev}, fShift);
				oShift = Geometry.getRotatedVector(oShift, iSignFlag * Math.PI / 2).apex;
				oCurr.x += oShift.x;
				oCurr.y += oShift.y;
				aNewSteps.push(oCurr);
